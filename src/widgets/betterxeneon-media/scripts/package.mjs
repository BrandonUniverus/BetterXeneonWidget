// Build orchestrator: runs `vite build` (so the .icuewidget always contains
// the latest source — earlier we'd ship stale dist/), then copies the
// iCUE-required sidecar files (manifest.json, translation.json, resources/),
// bumps the manifest patch version with a build timestamp so iCUE treats
// each install as an upgrade, and finally runs the `icuewidget` CLI to
// validate and package. Produces <id>.icuewidget at repo root /dist.
...
// Stamp the staged manifest with a build-unique patch version so iCUE
// treats each fresh install as an upgrade rather than "already installed".
// Format: <major>.<minor>.<unix-seconds>. The icuewidget CLI's validator
// only accepts strict X.Y.Z semver — no pre-release suffixes — so we burn
// the patch number with a monotonically increasing integer.
