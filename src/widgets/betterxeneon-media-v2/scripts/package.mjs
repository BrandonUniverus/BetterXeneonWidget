// Same minimal packager as the flicker-test widget. No Vite, no bundler —
// index.html is static HTML/CSS/JS. Patch version stamped with Unix seconds
// so iCUE treats every build as an upgrade.
...
// Must NOT start with a dot — the icuewidget CLI silently filters paths
// whose ancestors begin with `.` and writes an empty zip.
